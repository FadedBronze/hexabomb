...
find_available_discovery_port :: proc() -> (ok: bool, discoveryPort: int, discovery: net.UDP_Socket) {
...
init_base :: proc(network: ^NetworkBase, port: int, singleMachineTesting: bool) -> (success: bool) { 
    addr := net.IP4_Loopback
    ...
    ok, discoveryPort, discovery := find_available_discovery_port()
...
poll_message_ :: proc(packet: ^$T, socket: net.UDP_Socket) -> (err: PollErr) {
    buf: [1024]u8
...
broadcast_discovery_packet :: proc(
    ...
    broadcast := apply_subnet_mask(base.myEndpoint.address.(net.IP4_Address), base.mask)

    for DISCOVERY_PORT in DISCOVERY_PORTS {
        net.send_udp(base.discovery, payload, {
            address = singleMachineTesting ? net.IP4_Address{127, 0, 0, 255} : broadcast,
            port = DISCOVERY_PORT,
        })
    }
...
broadcast_packet :: proc(
    ...
    _, err_ := net.send_udp(base.socket, payload, target)
